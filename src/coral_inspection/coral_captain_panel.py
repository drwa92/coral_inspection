...
"""
Coral Captain RQT Panel

Mission console for your coral LLM + executor stack.
...
- Executor log (clean text from /rosout)          <- /rosout
"""
...
# ---------------------------------------------------------------------------
# Simple XY mini-map widget (dynamic scaling)
# ---------------------------------------------------------------------------
class XYMapWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        label_rect_y = QtCore.QRect(rect.left() + 2, rect.top(), 40, rect.height())
...
# Main RQT panel
# ---------------------------------------------------------------------------
class CoralCaptainPanel(Plugin):
    ...
    def __init__(self, context):
        super(CoralCaptainPanel, self).__init__(context)
        self.setObjectName("CoralCaptainPanel")
        ...
        if _HAS_BRIDGE:
            ...
            rospy.loginfo("[CoralCaptainPanel] cv_bridge OK, subscribing to cameras:")
            ...
            rospy.logwarn("[CoralCaptainPanel] cv_bridge not available; camera views disabled.")
        ...
        # Fixed compact size
        self._widget.setFixedSize(1200, 650)
    ...
    # ------------------------------------------------------------------
    def _build_ui(self):
        # Grid layout: 2 rows x 3 columns (all equal)
        grid = QtWidgets.QGridLayout(self._widget)
        ...
        self._prompt_history.setMinimumHeight(60)
        ...
        # ====== Bottom-middle: Vehicle State & XY Position ======
        vehicle_box = QtWidgets.QGroupBox("Vehicle State  /  XY Position")
        ...
        common_min_height = 280
        for box in [mission_box, memory_box, front_cam_box,
                    log_box, vehicle_box, down_cam_box]:
            box.setMinimumHeight(common_min_height)
    ...
    # ------------------------------------------------------------------
    # Timer tick – update text UI (non-blocking)
    # ------------------------------------------------------------------
    def _on_timer_tick(self):
        self._update_exec_status_view()
        self._update_memory_view()
        # pose & cameras are already live via signals
    ...
    def _on_send_prompt(self):
        ...
        if not text:
            QtWidgets.QMessageBox.warning(
                self._widget, "Empty prompt", "Please enter a prompt first."
            )
            return
        ...
        rospy.loginfo("[CoralCaptainPanel] Prompt sent: %s", text)
    ...
    def _on_execute_plan(self):
        if self._exec_client is None:
            try:
                rospy.loginfo("[CoralCaptainPanel] Waiting for execute service: %s",
    ...
    def _on_emergency_stop(self):
        ...
        if self._hold_client is None:
            try:
                rospy.loginfo("[CoralCaptainPanel] Waiting for hold service (Hold): %s",
    ...
    def _on_front_image(self, msg):
        ...
        try:
            ...
            rospy.logwarn("[CoralCaptainPanel] Error converting front image: %s", e)
    ...
    def _on_down_image(self, msg):
        ...
        try:
            ...
            rospy.logwarn("[CoralCaptainPanel] Error converting down image: %s", e)
